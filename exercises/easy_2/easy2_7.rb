...
## Problem ##
# input - array `arr` and an object `obj` ("of some sort")
# output - `obj`
# rules
#   1) accept a block and yield each element to the block, along with `obj`
#   2) return `obj`
#   3) not clear what classes of objects we have to support for `obj` -- design decision

def each_with_object(arr, obj)
  return nil unless [Array, Hash].include?(obj.class)
...
result = each_with_object([1, 2, 3], 0) do |value, obj|
  obj = value
end
...
# 1) initialize a new object to the value of `obj`
# 2) return nil if the class of `obj` is anything other than array or hash (only support arrays and hashes to start)
# 3) iterate over the input array. on each iteration:
#     > yield to the block
# 4) return `obj`